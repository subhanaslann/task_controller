...
void main() {
  ...
  setUp(() async {
    ...
    expect(loginResponse.user.role, anyOf(UserRole.teamManager, UserRole.admin));
  });
  ...
  group('ADMIN_FLOW_01: User Management Flow', () {
    test('should list all users in organization', () async {
      ...
      if (response.users.length > 1) {
        ...
        expect(
          response.users.every((u) => u.organizationId == orgId),
          true,
        );
      }
  ...
  group('ADMIN_FLOW_02: Topic Management Flow', () {
    ...
    test('should delete topic', () async {
      ...
      // Assert - Verify deletion (should return 404 on get)
      try {
  ...
  group('ADMIN_FLOW_05: Role Restrictions Flow', () {
    ...
    test('should prevent team manager from creating another team manager', () async {
      // Act & Assert
      try {
        await apiService.createUser(
          CreateUserRequest(
